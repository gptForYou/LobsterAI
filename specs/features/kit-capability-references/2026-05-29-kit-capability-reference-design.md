...
当前 Kit（专家套件）已经支持在商店中安装，并在对话输入区选择后自动展开为一组 Skill 注入到 Cowork 会话中。这个实现解决了"一键安装和使用一组 Skill"的问题，但在消息展示层暴露了一个体验问题：

- 用户选择的是一个 Kit，例如"GitHub 专家套件"
- 运行时为了执行能力，会把 Kit 展开成多个底层 Skill
...
2. 保留当前 Kit 展开为 Skill 并注入 OpenClaw 会话的运行时能力。
...
**Then** runtime 仍收到展开后的 Skill 列表，能力执行不退化。
...
- 调用 OpenClaw runtime 时继续传递完整 `skillIds`，保证现有 Skill 注入行为不变。
...
3. 根据 `allSkillIds` 构建 inlined skill prompt
...
4. 构建 runtime Skill 列表：
   - `directSkillIds + resolvedKitCapabilities.skillIds`
5. 构建 `skillPrompt` 时使用 runtime Skill 列表。
...
```typescript
runtime.startSession(session.id, prompt, {
  skillIds: [
    ...(selection.skillIds ?? []),
...
| `src/renderer/components/cowork/CoworkPromptInput.tsx` | 提交时区分直接 Skill、Kit 引用、运行时能力解析 |
...
| `src/main/main.ts` | Cowork IPC options 接收 capability selection；写入用户可见 metadata；runtime 调用仍传完整 Skill 列表 |
...
| 多个 Kit 包含同一个 Skill | runtime `skillIds` 去重，展示仍显示两个 Kit 引用 |
...
5. OpenClaw runtime 仍收到 Kit 展开后的完整 Skill 列表，现有 Kit 能力不退化。
...
3. 确认实际回复仍能调用 Kit 对应 Skill 的能力。
4. 同时选择 Kit 和一个独立 Skill，确认展示和能力都正确。
5. 重新编辑该消息，确认 Kit 和独立 Skill 都恢复。
6. 重启应用后打开历史会话，确认 Kit 引用展示稳定。
